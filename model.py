...
def rnn_depth_net_encoderlstm(current_input,is_training=True):
    ...
    with tf.variable_scope('rnn_depth_net', reuse = tf.AUTO_REUSE) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            #c5=tf.squeeze(cnv5,1)
            #cnv5b, hidden5 = convLSTM(cnv5, hidden_state[4], 512, [3, 3], scope='cnv5_lstm')

            for i in range(current_input.get_shape()[1].value):
                cv5=tf.slice(cnv5,[0, i,0, 0, 0],
                             [-1,1 , -1,-1, -1])
                cv5=tf.squeeze(cv5,1)
                upcnv5 = slim.conv2d_transpose(cv5, 512, [3, 3], stride=1)
                upcnv5 = resize_like(upcnv5, cv5)
                upcnv5 = tf.layers.batch_normalization(upcnv5, training=is_training)
                i5_in  = tf.concat([upcnv5, cv5], axis=3)
                icnv5  = slim.conv2d(i5_in, 512, [3, 3], stride=1, scope='icnv5')
                icnv5 = tf.layers.batch_normalization(icnv5, training=is_training)

                cv4= tf.slice(cnv4, [0, i, 0, 0, 0],
                               [-1, 1,-1, -1, -1])
                cv4 = tf.squeeze(cv4,1)
                upcnv4 = slim.conv2d_transpose(icnv5, 256, [3, 3], stride=2, scope='upcnv4')
                upcnv4 = resize_like(upcnv4, cv4)
                upcnv4 = tf.layers.batch_normalization(upcnv4, training=is_training)
                i4_in  = tf.concat([upcnv4,cv4], axis=3)
                icnv4  = slim.conv2d(i4_in, 256, [3, 3], stride=1, scope='icnv4')
                icnv4 = tf.layers.batch_normalization(icnv4, training=is_training)

                cv3b = tf.slice(cnv3b, [0, i,0, 0, 0],
                               [-1, 1,-1, -1, -1])
                cv3b = tf.squeeze(cv3b,1)
                upcnv3 = slim.conv2d_transpose(icnv4, 128,  [3, 3], stride=2, scope='upcnv3')
                upcnv3 = resize_like(upcnv3, cv3b)
                upcnv3= tf.layers.batch_normalization(upcnv3, training=is_training)
                i3_in  = tf.concat([upcnv3,cv3b], axis=3)
                icnv3  = slim.conv2d(i3_in, 128, [3, 3], stride=1, scope='icnv3')
                icnv3 = tf.layers.batch_normalization(icnv3, training=is_training)

                cv2b = tf.slice(cnv2b, [0, i, 0, 0, 0],
                               [-1, 1, -1, -1, -1])
                cv2b = tf.squeeze(cv2b,1)
                upcnv2 = slim.conv2d_transpose(icnv3, 64,  [3, 3], stride=2, scope='upcnv2')
                upcnv2 = resize_like(upcnv2,cv2b)
                upcnv2 = tf.layers.batch_normalization(upcnv2, training=is_training)
                i2_in  = tf.concat([upcnv2, cv2b], axis=3)
                icnv2  = slim.conv2d(i2_in, 64, [3, 3], stride=1, scope='icnv2')
                icnv2 = tf.layers.batch_normalization(icnv2, training=is_training)
                ...
                upcnv1 = tf.layers.batch_normalization(upcnv1, training=is_training)
                i2_in = tf.concat([upcnv1,cv1], axis=3)
                icnv1  = slim.conv2d(i2_in, 32,  [3, 3], stride=1, scope='icnv1')
                icnv1 = tf.layers.batch_normalization(icnv1, training=is_training)
